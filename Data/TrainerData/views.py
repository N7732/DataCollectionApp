...
class ExternalAPIKeyReadOnlyPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # 2. If the user provides a valid API key, grant full access (GET, PATCH, DELETE)
        api_key = request.headers.get('X-API-KEY')
        if api_key and api_key.startswith('sk_'):
            return True
            
        # 3. If it's a Django staff/admin (via browser session), allow full access
        if request.user and request.user.is_staff:
            return True
            
        raise PermissionDenied("A valid API Key (X-API-KEY header) is required to access trainer data.")
